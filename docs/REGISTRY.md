...
앱끼리 주고받는 유일한 길이다 ([RULES.md › W-01](../RULES.md#w-01-폴더--앱--명령)). 읽기 전용 · 표준 출력에 UTF-8 JSON · `format` 으로 형식 버전을 밝힌다.
형식을 바꾸는 앱이 `format` 을 올리고, 부르는 앱은 모르는 `format` 이면 그 기능만 끈다. 검사기는 여기 없는 앱 사이의 참조와, MANUAL 에 없는 공개 명령을 위반으로 본다.

| 앱 | 명령 | 내주는 것 | 형식 | 쓰는 앱 |
|---|---|---|---|---|
| `secretary-1` | `secretary-1.py --export-events --from YYYY-MM-DD --to YYYY-MM-DD` | 기간 안의 일정 — id · 제목 · 시각 · 장소 · 종일 · 반복 (메모는 안 줌) | 1 | — |

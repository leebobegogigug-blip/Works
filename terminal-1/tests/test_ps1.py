"""terminal-1.ps1 실제 실행 테스트 (pwsh 가 있는 Linux/macOS 에서만 · Windows 는 CI 에서 문법만 검사)

wt · opencode 는 가짜 실행 파일로 바꿔서, terminal-1 add 가 Windows Terminal 에 넘기는 명령줄을 그대로 받아 본다.
PWSH 환경변수로 pwsh 경로를 줄 수 있다."""
import json, os, shutil, stat, subprocess, sys, tempfile, time, unittest

HERE = os.path.dirname(os.path.abspath(__file__))
PS1 = os.path.join(HERE, "..", "terminal-1.ps1")
PWSH = os.environ.get("PWSH") or shutil.which("pwsh")


@unittest.skipUnless(PWSH and os.name != "nt", "pwsh 가 없음 (Windows 는 CI 에서 문법 검사)")
class Ps1(unittest.TestCase):
    def setUp(self):
    ...
    def registry(self):
        with open(os.path.join(self.appdata, "terminal-1", "instances.json"), encoding="utf-8-sig") as f:
            data = json.load(f)
    ...
    def test_add_ls_rm_and_password_file(self):
        ...
        pw = os.path.join(self.appdata, "terminal-1", "server-password")

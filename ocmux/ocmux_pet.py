"""
ocmux_pet.py - TOKEN QUEST: 토큰펫 게임 엔진 (화면 없음 / 순수 로직 + 저장)
...
다마고치 + RPG:
  ...
  - 저장: %LOCALAPPDATA%\\ocmux\\pet-<이름>.json (+ 다른 창용 live 요약, 이벤트 버스, 중복 실행 잠금)
"""
import collections
import datetime
import difflib
...
def data_dir():
    ...
    return os.path.join(base, "ocmux")
...
class PetGame:
    ...
    def _load_or_create(self, name, now):
        ...
        if status == "ok" and valid and s["v"] > D.SAVE_VERSION:
            ...
            self.note("더 새로운 버전의 ocmux가 만든 저장 파일이에요. 덮어쓰지 않도록 관전 모드로 열었어요")
            return
        if status == "locked":
...
def compose_reaction(scope, text):
    ...
    if not sm or sm.get("form") == "egg":
        return ""
    from ocmux_term import rgb, RST
...
def pet_badge(scope, width=60):
    ...
    if not sm:
        return ""
    from ocmux_term import rgb, RST, B, P3, chip

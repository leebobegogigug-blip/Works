...
| command | does |
|---|---|
| `ocmux add` | current folder → new channel (free port from 4096) |
| `ocmux add C:\work\web -Name web` | another project |
| `ocmux add C:\work\api -Headless` | hidden `opencode serve` + attached TUI (server survives closing the tab) |
| `ocmux add -PetName 코코` | name the new pet egg (default 토큰이) |
| `ocmux overview` | reopen channel 00 (opens automatically on the first add) |
| `ocmux ls` | channel table with state LEDs |
| `ocmux focus api` | reopen a closed channel on the same port |
| `ocmux rm api` | unregister (stops a headless server; the pet's save stays) |
| `ocmux prune` | drop offline channels |
| `ocmux setup` | install the `ocmux Black` color scheme |
...
`-GameWidth 0.58` `-ComposeHeight 0.30` `-Compact` (no bottom row) `-NoLogs` `-NoCompose`
...
| 7 | **one icon map** — status symbols come only from the table below | events, sessions, pet, ranch, `ocmux ls` |
...
- **logs** (`oc_monitor.py logs`): the same log formatting as the LOGS regions
...
- tuning: `STORY` (`fast` = chapters open at once, `every` = days per chapter) and `CHAPTERS` in `ocmux_pet_data.py`.
...
### 07.8 save files
- `%LOCALAPPDATA%\ocmux\pet-<name>.json` · `.live.json` (for other panes) · `bus-<name>.jsonl` · `raid-<name>.json`
...
- registry `%LOCALAPPDATA%\ocmux\instances.json` (name, port, color, **ch**)
- headless logs `%LOCALAPPDATA%\ocmux\logs\<name>.log`
...
- color scheme fragment `%LOCALAPPDATA%\Microsoft\Windows Terminal\Fragments\ocmux\ocmux.json` (only ocmux tabs use it)
...
- tests (standard library `unittest`, 101 tests — game engine, opencode signal bridge, save/raid edge cases, main story, token taper):
  `py -3 -m unittest discover -s tests`
- one-frame snapshot of any pane, for screenshots or checks without a live terminal:
  `py -3 oc_monitor.py rpg --name demo --once 1 --cols 80 --rows 24` (add `--guide` to show the `?` guide)
- `oc_monitor.py` modes: `status` `overview` `logs` `usage` `rpg` `compose` — `py -3 oc_monitor.py -h` lists every flag

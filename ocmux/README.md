...
ocmux가 쓰는 한글, 선 문자(`┌─┐`), 블록(`▁▅█`), 점자 스피너, 아이콘이 전부 들어 있어서 다른 글꼴로 대신 그려지는 글자가 없습니다.
...
- Unifont는 `√ ‼ ⊠ ☼` 같은 기호를 폭 1칸으로 그리지만, `✓ ⚠ ★` 같은 기호는 폭 2칸으로 그립니다. 폭 2칸 기호는 옆 글자와 겹치기 때문에 ocmux는 화면 아이콘을 전부 폭 1칸 기호로만 씁니다. 다른 글꼴로 바꿔도 그대로 잘 보입니다.
...
모든 창이 같은 규칙 일곱 가지를 따릅니다. 일정 비서 [jaba](https://github.com/leebobegogigug-blip/jaba) 도 같은 규칙 · 같은 팔레트를 씁니다 (works 시스템).
...
네이비가 주색입니다. 번호 배지(`01`) · 칸 이름 칩(`USAGE` `COMPOSE` `OCMUX`) · 채널 01 의 탭 색은 네이비이고,

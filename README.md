...
- **로컬 학습** — "앞으로 스크럼은 15분으로 잡아" 또는 `/학습 …` → `jaba_rules.json`에 저장, 모든 제안·정리에 우선 적용
- **윈도우 알림** — 장소 있는 일정 15·5·1분 전, 장소 없는 일정 5·1분 전
- **내 PC 전용** — `127.0.0.1`에만 열리고 대화는 저장하지 않습니다. 밖으로 나가는 통신은 설정한 LLM 주소뿐
...
- 빠른 키 **Alt+1~4** · 오늘 일정 펼치기 **Alt+D** · 학습 서랍 **Alt+M**
- 명령어: `/학습 <규칙>` · `/잊어 r3` · `/규칙` · `/알림` · `/도움`
...
`config.json`(API 키) · `jaba.db`(일정) · `jaba_rules.json`(학습 규칙) · `jaba.bat`(PC별 경로) — 모두 `.gitignore`에 들어 있습니다.
